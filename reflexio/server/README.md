...
**User-playbook exposure boundary**: Authenticated enterprise deployments install `ExposureLedgerRecorder` through their capability registry. In those deployments, `POST /api/search` and `POST /api/search_user_playbooks` synchronously record the final served user-playbook set through `services/search_exposure.py` before metering or releasing success, and recorder failures fail closed. Shared `create_app()` intentionally installs no default recorder; local/no-auth OSS and custom shared app constructions persist exposures only when their deployment registers `SEARCH_EXPOSURE_RECORDER`. Because that absence is legal, `record_search_exposures()` reports it rather than raising: it returns `SearchExposureOutcome.RECORDED`, `SearchExposureOutcome.NO_RECORDER`, or `SearchExposureOutcome.UNCORRELATED`, so callers that depend on the batch reaching the ledger (corpus reconstruction, replay tooling) can assert on the outcome instead of assuming a silent no-op was a durable write. The search routes deliberately ignore the outcome -- they cannot distinguish a supported recorder-less deployment from an enterprise misconfiguration. The direct route skips recording when its final result set is empty.

A batch carrying neither `request_id` nor `session_id` is refused before any recorder is consulted, and reports `SearchExposureOutcome.UNCORRELATED`. Those two columns are the only correlation the ledger stores -- `interaction_id` is not a column, it only seasons the deterministic event id -- so the guard is exactly the schema's own `missing_correlation` reason, which fires when both are blank. Such a row can never bind a served playbook to a session: `request_id` is the only key the evidence loader resolves a session and user from, and there is no reverse lookup from a session. It is not inert, though -- every reconstructability gate counts it in the denominator and never in the numerator, and exposure is append-only, so one uncorrelated retrieval would permanently lower the org's ratio with no product path to undo it.
...
- **Two-level retry model**: transport failures/timeouts walk a reflexio-owned per-rung fallback ladder within a generation turn (`num_retries=0`, primary then configured fallbacks) — the client rebuilds request params for each rung itself rather than delegating to LiteLLM's native `fallbacks` kwarg. Structured-output repair is across turns and is opt-in via `structured_output_validator`. Callers without a validator keep their single same-model parse retry, and that retry is **corrective whenever the parse failure carried validation errors**: the rung's original messages plus a bounded turn naming the failing field paths and pydantic error tags (`decision: missing`, `manifest_role: extra_forbidden`, ...). It never echoes the model's own output — unlike the validator path's repair turn — and it re-keys `extra_headers["Idempotency-Key"]` via `structured_output_repair_idempotency_key` so a provider honouring the header cannot replay the rejected answer. The validator path's repair turn is re-keyed the same way, being a strictly larger change to the request. A parse failure that names nothing — the deliberate "appears truncated" diagnostic — still re-issues identically, key included: its params are rebuilt from the caller's kwargs rather than reusing the dict attempt 1 handed the transport, so a provider that wrote its own `Authorization`/`Content-Type` into that dict cannot have them re-sent as caller intent. A `provider_request_guard` can recognise the corrective shape with `is_structured_output_correction_turn`.
...
- **External source admission**: `services/external_admission.py` defines the optional library-only transaction participant. Its claim and receipt writes join the canonical storage commit scope; exceptions roll back all writes. Exact replay returns original IDs without repeating evaluation scheduling. The default publish API is unchanged.
