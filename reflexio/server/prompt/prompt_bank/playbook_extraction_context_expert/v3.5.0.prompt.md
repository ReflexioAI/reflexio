...
- Each entry must satisfy:
  - `trigger` must describe a problem/situation, not a user preference. It must be evaluable at the earliest moment the policy should apply: do not narrow it by "and"-conjoining incidental details from this specific exchange (identifiers, one-off values); phrase alternate symptoms as "or ..." broadenings instead
