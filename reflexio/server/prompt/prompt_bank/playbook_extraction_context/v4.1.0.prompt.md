...
changelog: "v4.1.1: adds resumable extraction guidance, preserves final verified implementation details, and keeps action-vs-avoidance framing without requiring a polarity output field."
variables:
...
Use the final verified implementation state as the source of truth. If a code detail, parameter, command, or assumption was later changed after a failed check or self-correction, do not present it as positive guidance; omit it or capture it as negative/avoid evidence.
...
Return only a JSON object with a single key `"playbooks"` whose value is a list of zero or more playbook entries. Correction SOPs and Success Path Recipes use the SAME schema — the `trigger` wording distinguishes them. Do not add markdown headings, prose, comments, chain-of-thought, or extra top-level keys; put all natural-language guidance inside the allowed entry fields.
...
* **Agent successfully produced a structured deliverable from source material after discovering the decisive selection and validation criteria.**
* **Output:**
{{
  "playbooks": [
    {{
      "rationale": "Captures the selection and validation structure that the original trajectory converged to after a broad-search detour.",
      "trigger": "Producing a structured deliverable from source material when the correct output depends on selection criteria, transformation steps, and validation checks.",
      "content": "(1) Start from the source artifact that contains the authoritative criteria. (2) Extract the selection rules and required output structure. (3) Apply the transformation in the order the criteria require. (4) Validate the result against the narrow acceptance checks before broad review. Detour to skip: do not begin with broad exploration once the authoritative criteria source is known."
    }}
  ]
}}
