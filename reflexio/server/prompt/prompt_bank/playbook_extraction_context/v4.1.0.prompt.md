...
changelog: "v4.1.0: adds resumable extraction guidance, compact task recipes, and action-vs-avoidance framing without requiring a polarity output field."
variables:
...
* Each entry in `"playbooks"` MUST satisfy ALL of the following:
  ...
  * "polarity" is REQUIRED — `"positive"` (default) or `"negative"` per the Per-rule polarity section
