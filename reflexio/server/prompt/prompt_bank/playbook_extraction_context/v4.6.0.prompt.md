...
changelog: "v4.6.0: restores the procedural Correction SOP and Success Path Recipe structure from production v4.5.0 while adding the strict local-turn evidence contract, durable preferences, answered-clarification/non-delivery/duplicate distinctions, and narrow guards against unsupported clauses, causality, unseen replacements, internal-status value claims, and invented avoidance guidance. The first pass enumerates supported lessons; the reviewer remains the semantic precision gate."
variables:
...
`[T#]` marks a visible turn or event; neither is a database id. Use `[T#]` only
as `evidence.turn_ref`. Never place a local label in `rationale`, `trigger`, or
`content`.
...
7. Cite the shortest verbatim spans that establish the behavior and correction. Confirm each `[T#]` exists and each span occurs exactly in that turn.
...
6. Cite the shortest verbatim spans that establish the decisive action and visible outcome. Confirm each `[T#]` exists and each span occurs exactly in that turn.
...
Each playbook object MUST include exactly non-empty `evidence_kind`, `rationale`, `trigger`, `content`, and `evidence`. `evidence_kind` is one of `correction`, `preference`, `observed-failure`, `rejected-approach`, or `verified-success`. `evidence` contains one or more `{{"turn_ref": "T#", "source_span": "short verbatim excerpt"}}` objects. Do not add database ids, markdown headings, prose, comments, chain-of-thought, or extra keys. Local labels may appear only in `evidence.turn_ref`.
...
`evidence` array is the only place for turn references.
...
* Each entry in `"playbooks"` MUST satisfy ALL of the following:
  ...
  * "evidence" is REQUIRED — one or more allowed local turn references with exact verbatim spans
