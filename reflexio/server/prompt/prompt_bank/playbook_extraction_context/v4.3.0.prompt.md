...
Make triggers **retrieval-general** and content **action-specific**: replace one-off filenames, ports, branches, users, and exact repo paths with their reusable role unless they are essential search keys; keep concrete downstream surfaces, checks, and avoid-detours in `content`.
...
Compactness rule: generalize the trigger one level above the exact incident, but do not blur away operational surfaces in `content`. "Changing a service port behind a load balancer" is retrievable; "deployment config changed" is too vague.
