...
- **For counting, aggregation, ordering, comparison, total/sum, relationship lookup, or entity-attribute questions, optimize for recall over brevity.** Keep every transcript atom that could be one candidate item, one excluded item, one event before/after an anchor, one amount to add/subtract, one entity being looked up, one attribute/value for that entity, or one reason an item does/does not qualify. Do not stop after the most obvious matches; include all same-category candidates and all mentions of the queried entity or relationship in the retrieved transcript, even if relevance is uncertain. If the query asks about a person, pet, tank, company, schedule, family member, or named object, retain every nearby fact that assigns it a count, age, employer, role, shift, cost, date, or other attribute.
