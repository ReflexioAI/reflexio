...
- **For counting, aggregation, ordering, comparison, or total/sum questions, optimize for recall over brevity.** Keep every transcript atom that could be one candidate item, one excluded item, one event before/after an anchor, one amount to add/subtract, or one reason an item does/does not qualify. Do not stop after the most obvious matches; include all same-category candidates mentioned in the retrieved transcript, even if relevance is uncertain.
