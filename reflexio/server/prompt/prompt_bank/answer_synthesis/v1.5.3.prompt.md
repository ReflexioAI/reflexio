...
9a. **List-shape questions (CRITICAL — multi_hop fixer).** When the question's expected answer is a SET of items (not a count, not a single fact), the model must surface EVERY retrieved atom that names an instance of the requested category. This is the multi_hop family: "which X did the user mention?", "what books / cities / sports / collectibles / bands has the user X?", "which X besides Y?", "what other X?".

   **Structural test for list-shape:** the answer-noun is plural OR the question is enumerative even when phrased singularly with exclusion ("besides X", "other than Y"). Examples that fire this rule:
   - "Which sports does <user> like besides basketball?" → the answer is "the other sports", a list.
   - "What books has <user> read?" → list of titles.
   - "What items does <user> collect?" → list of categories.
   - "Which authors has <user> read?" → list of authors (even when only inferable from book atoms).

   **Mandatory enumeration procedure** (do NOT collapse to single best match):
   1. Identify the category C the question asks about (sports, books, cities, authors, items collected, bands, ...).
   2. Scan ALL retrieved profiles and excerpts. For each profile/excerpt, check: does it name an instance of C?
   3. List EVERY distinct named instance found. Use one bullet per instance, with profile_id citation:
      - `- <instance-name>` (cite `profile_id`)
   4. State the answer as the list.

   **Strict qualifier filter (over-inclusion fix).** When the question carries a qualifier ("non-basketball goals", "career goals not related to basketball", "sports besides basketball", "books read during recovery"), apply the qualifier strictly: include only instances that satisfy the qualifier. Do NOT include thematically-related instances that DON'T satisfy the qualifier just because they're retrieved.
   - Example: "John's goals not related to basketball" — INCLUDE: endorsements, brand, charity. EXCLUDE: shooting percentage, championship (these are basketball-related).
   - Example: "books Tim read besides Harry Potter" — EXCLUDE: Harry Potter atoms even when they're retrieved.

   **Inferred-attribute aggregation** (multi_hop authors-from-books pattern). When the question asks for a category C that's inferable from a different-but-related category D the user mentioned, infer the C-instance from each D-atom and list it:
   - "Which authors has <user> read?" — if profiles only mention book titles, infer the author for each known title and list authors. Cite the book-naming profile_id (you're answering an inferred fact from the source data).
   - "Which directors has <user> watched?" — same pattern from movie atoms.
   - Constraint: only infer when the mapping (book→author, movie→director, etc.) is a well-known fact you'd produce from world knowledge. Do NOT invent — if uncertain about a mapping, omit that instance.

   **Under-inclusion is the dominant failure for list-shape questions.** When in doubt, include MORE candidates rather than fewer. The user can filter; missing items can't be recovered. Compare against rules 12-14 (instance disambiguation) which apply to single-instance questions — those rules do NOT govern list-shape, where the answer IS the union of valid instances.
