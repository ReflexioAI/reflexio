...
Rehydration use: ALWAYS — no exceptions. Every aggregation/counting/total question MUST call `read_session_text` once with up to 4 relevant `session_id`s from qualifying/near-miss topic hits before finishing. Do not assess whether rehydration is needed — the profile cache is lossy by design (countable atoms are split, ranked imperfectly, and may aggregate or split actions inconsistently). Raw turns are the authoritative source for counts. The `read_session_text` call is cheap; `finish`-without-rehydrating is the failure mode this rule eliminates. After rehydrating, count from raw-turn evidence plus profiles together, deduping carefully.
