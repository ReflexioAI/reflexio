...
Rehydration use: MANDATORY for aggregation questions. Always call `read_session_text` with up to 4 relevant `session_id`s from qualifying/near-miss topic hits; then count from raw-turn evidence plus profiles, deduping carefully. Skip ONLY when a single retrieved profile directly states the exact total verbatim (e.g. "user has 17 fish total") — not when you've assembled a partial enumeration from multiple profiles. **Self-check before finishing**: if your retrieved enumeration is suspiciously short (1-3 items when the question implies a larger set), or the question asks for a total across distinct containers/categories/locations, treat the enumeration as incomplete and rehydrate. Do not finish with a low count just because the retrieved profile set was small — the bi-encoder may have missed numeric-bearing atoms.
