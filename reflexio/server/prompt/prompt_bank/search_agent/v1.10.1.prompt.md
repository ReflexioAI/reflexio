---
active: false
description: "Agentic search — orchestration patterns + cross-encoder rerank + LLM rerank for brand→category gaps + storage_stats; reads only. v1.10.1 raises Pattern D session_id cap from 4 to 6. DEACTIVATED after r109 — net regression on tune-100 (M-S +17pt offset by K-U -24pt; loop-decision variance hits K-U disproportionately)."
variables:
  - query
  - max_steps
  - enable_agent_answer
...
2. **MANDATORY rehydration**: identify ALL distinct `session_id`s present in step 1's top-20 hits, then call `read_session_text(session_ids=[<every distinct session_id, up to 6>])` in a single call. Counting questions span multiple sessions; passing only ONE `session_id` will miss candidates in other sessions. The cap is 6 (raised from 4 in v1.10.0) because 5+ atom counting questions are common — capping too low leaves candidate atoms in untouched sessions. If step 1 returns more than 6 distinct session_ids, prioritize those whose top-ranked profile is most directly on-topic. Do not skip this step.
...
Rerank: prefer `rerank=False` on the verbatim first pass; if the targeted reformulation still leaves an operand missing, prefer `rerank=True, refine_with=<the missing operand named explicitly>` to push numeric atoms over surrounding narrative. **Exception — superlative-within-category** ("which [category] did I spend the most/least at"): set `llm_rerank=True` on the wide first pass when the answer is likely a brand/proper-noun whose profile content does NOT contain the literal category keyword; this bridges brand→category so the comparison includes ALL category instances, not only those the bi-encoder recognised.
