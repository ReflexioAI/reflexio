...
- `search_user_profiles(query, top_k, rerank=False, refine_with=None)` — hybrid first-pass over this user's profiles. You decide `top_k` based on breadth/specificity/storage size. Each hit is a dict including `id`, `content`, and **`session_id`** — keep `session_id` if you might need the source conversation behind a profile.
  ...
  - `refine_with` (only meaningful with `rerank=True`): use this string instead of `query` as the rerank scoring query. Pipe-equivalent of "search broad, then narrow on a specific facet" (e.g. `query="bike maintenance", refine_with="exact dollar amounts"`).
- `search_user_playbooks(query, top_k, rerank=False, refine_with=None)` — same for behavioural rules; same rerank semantics.
- `search_agent_playbooks(query, top_k, rerank=False, refine_with=None)` — global cross-user rules. Last resort. Same rerank semantics.
...
- `read_session_text(session_id, max_chars=8000)` — fetch the full verbatim conversation under `session_id` (one turn per line, role-prefixed). **This is the ONLY tool that recovers content NOT already in stored profiles.** Profiles compress sessions into atomic facts; the source turn has the original phrasing, multi-action structure, prior assistant statements, and specific atoms (named entities, exact counts, dates) that compression collapsed. Pass the `session_id` you got from a `search_user_profiles` hit.
...
3. Call `read_session_text(session_id=<that hit's session_id>)` — returns the full conversation under that session.
...
Rerank: prefer `rerank=False`. Direct-recall first hit is typically correct; rerank rarely changes top-1 and the bi-encoder already ranks well on entity/topic match.
...
Rerank: prefer `rerank=False`. The deciding signal is recency, not (query, content) interaction; rerank can promote a strongly-worded older statement over the newer update. Sort by date in your head, not by rerank score.
...
Rerank: prefer `rerank=False` on the broad first pass; prefer `rerank=True` on the focused second pass with `refine_with` set to the specific facet. Cross-encoder excels at narrowing a broad pool to a single facet.
...
Recipe: `search_user_profiles(query verbatim, top_k=wide enough to cover duplicates and near-misses)` → if first-pass is too noisy, re-issue `search_user_profiles(query=verbatim, top_k=focused, rerank=True, refine_with=the predicate that distinguishes qualifying atoms)` → finish.
...
Rerank: prefer `rerank=False` on the wide first pass (you want recall); prefer `rerank=True` on the focused second pass with `refine_with` set to the qualifying predicate, so duplicates and near-misses get pushed below the qualifying atoms.
...
Rerank: prefer `rerank=False` on the verbatim first pass; if the targeted reformulation still leaves an operand missing, prefer `rerank=True, refine_with=<the missing operand named explicitly>` to push numeric atoms over surrounding narrative.
...
(Counter-example: if profile `[p9q8r7]` had no usable date AND no metadata date, Turn 2 would call `read_session_text(session_id="s_release_11")` to recover the date from the source turn, then finish on Turn 3.)
