...
## Relevance gate before finish (mandatory)

Before calling `finish`, verify your retrieved evidence actually contains the answer. The most common failure mode is calling finish on the first search's results when those results don't contain the answer's atoms. To prevent that:

1. **Identify the answer's expected atoms** based on question shape:
   - "What X has Y done?" / "Which X did Y choose?" → expected atoms are specific named subtypes (e.g. for "martial arts" expect "kickboxing", "karate", "judo", "taekwondo", "MMA"; for "favorite cuisine" expect "Italian", "Thai", a restaurant name, etc.)
   - "When/where/how many" → expected atoms are dates, places, counts, quantities
   - "What is X's status / role / occupation?" → expected atoms are concrete labels ("single", "married", "engineer", a company name)
   - "What did X research / study / discuss?" → expected atoms are the topic/subject names

2. **Scan your top 3 retrieved snippets.** Do they contain those expected atoms (literally — do not accept paraphrase substitutions in this gate)?

3. **If NO**: do NOT call finish. Try a second-pass search:
   - For category questions (subtype atoms expected): call `search_user_profiles` AGAIN with subtype keywords inserted into the query (e.g. `query="kickboxing taekwondo karate judo MMA"` instead of `query="martial arts"`), or set `llm_rerank=True`.
   - For other shapes: try `refine_with=<expected atom name>`, or `read_session_text` on the most plausible session_id from your top hit.

4. **After this second-pass attempt**:
   - If your evidence now contains the atoms, finish.
   - If still no relevant evidence, finish — but only after using at least 3 of 5 budget steps. A single search returning off-topic content is under-investigation, not exhaustive search.

This gate intentionally OVERRIDES the "Limit follow-ups" guidance below when first results don't contain the expected answer atoms. The follow-up cost is small; the cost of a wrong "I don't have information" answer is large.
