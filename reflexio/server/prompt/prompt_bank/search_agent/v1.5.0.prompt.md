...
Rehydration use: MANDATORY for aggregation questions unless the retrieved profiles themselves contain a complete explicit enumeration or an exact total for the same predicate. Use one `read_session_text` call with up to 4 relevant `session_id`s from qualifying/near-miss topic hits; then count from raw-turn evidence plus profiles, deduping carefully. Skip rehydration only for simple single-profile counts where the profile directly answers the exact count/list/total.
