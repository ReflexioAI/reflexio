...
## Reasoning before finish (mandatory)

For every question, before calling `finish`, do this completeness check. This is intent-based, not keyword-based — it applies regardless of how the question is phrased ("how many" / "what people" / "tell me everyone" / "list of" / "X's friends" all map to the same underlying intent shape).

### 1. What shape would a complete answer take?

Reason about the question and decide:

- **Cardinality** — would a complete answer be a single item, multiple items, or an aggregate?
  - Single: "what is X's name", "when did Y happen", "where did Z go", "is X true", "what is X's [status / role / occupation / preference]"
  - Multi: "what people / things / items / activities did X engage with", "all the X that Y did", "X's friends", "list of Y" — anything that implies enumeration of distinct items, regardless of phrasing
  - Aggregate: "how many", "how much", "which had the most", "percentage / ratio / difference"

- **Atom shape** — what kind of token would identify each answer item?
  - Named entity (person / place / brand / song title / book title)
  - Specific subtype noun (e.g. "kickboxing" within "martial arts", "pendant" within "jewelry")
  - Number / date / quantity / duration
  - Concrete descriptive phrase the user said
  - Boolean / categorical label (single / married / employed)

### 2. Does your retrieved evidence contain those atoms?

For each expected atom in your hypothesis answer, scan your top retrieved snippets:
- Is there a literal token for this atom in any snippet (not a paraphrase)?
- For multi-item questions: count the **distinct** atoms across all retrieved profiles. Compare to the cardinality the question implies. If the question implies many but you have one or two, that's a gap.

### 3. Gap analysis — search for what's missing

When evidence falls short of expectation:

- **Multi-item with too few items found** — the missing items likely live in profiles that cluster on a DIFFERENT semantic axis than your first query. Try a search that targets a different facet of the same topic, NOT a paraphrase of the original query. Example: "people met while volunteering" → first query clusters on "met / volunteering" (catches one name); a second query clustering on a different facet ("shelter residents", "people user mentioned by name") will surface other names stored in differently-worded profiles. After 2-3 angles, aggregate the union.

- **Single-item with off-topic top profile** (top profile is roughly relevant but doesn't contain the expected atom):
  - For category↔instance gaps: `search_user_profiles(query=<atom keywords>, llm_rerank=True)` — bridges "martial arts" ↔ "kickboxing" / "taekwondo".
  - For operand-bearing details (the topic profile names the thing but lacks the specific operand the question asks about): `read_session_text(session_ids=[<top hit's session_id>], query=<expected atom keywords>)` — recovers operands compressed out of profiles (design motifs, exact names, multi-turn context).
...
Finish only when:
- Retrieved evidence supports the expected cardinality of atoms, OR
- You've made at least one targeted search per missing atom-angle within your budget, even if some atoms remain unfound.

Never finish on the first search's results if the cardinality of retrieved atoms falls short of what the question implies. The cost of "I don't have information" or partial answers is much higher than a few extra searches.

This reasoning step intentionally OVERRIDES the "Limit follow-ups" guidance below when first results fall short of expected cardinality.
