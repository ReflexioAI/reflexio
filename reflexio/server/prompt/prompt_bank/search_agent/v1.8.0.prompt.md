...
2. **MANDATORY rehydration**: `read_session_text(session_ids=[<top 4 distinct session_ids from step 1's hits, prioritising the ones whose profile content names a candidate atom>])`. Fetch raw turns BEFORE attempting to enumerate or count. Do not skip this step. The profile cache is lossy by design; raw turns are the source of truth for counts.
