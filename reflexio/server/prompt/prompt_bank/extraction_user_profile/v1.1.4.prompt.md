---
active: false
description: "Agentic extraction — UserProfile axis. v1.1.4 strengthens action-verb preservation on possession events: when the user reports an action they took on an object (replaced X, fixed X, donated X, got rid of X, upgraded X) the action verb itself is the count-relevant signal, not just the resulting state. Verified standalone but DEACTIVATED after r95 re-ingestion regression: re-extracting with v1.1.3 (which v1.1.4 builds on) gained framing-fact wins (3c1045c8 dept age, 8e91e7d9 siblings) but had stochastic side effects on agent-recommendation capture (-2 SS-A: lost Sugar Factory, Nostalgia). Awaiting investigation into extraction-time non-determinism before re-activating."
variables:
  - sessions
  - extraction_criteria
  - max_steps
...
5. **Countable items AND countable actions on possessions.** Each enumerable THING the user mentions becomes its own profile (toy, book, friend, restaurant). Never bundle items.

   **Action-verb preservation rule (MANDATORY).** When the user uses one of these verbs on an object — `replaced`, `fixed`, `repaired`, `upgraded`, `donated`, `got rid of`, `sold`, `swapped`, `tossed`, `trashed`, `had X repaired/serviced/cleaned/installed` — the profile MUST contain that verb. The action is the count-relevant signal; the resulting state is NOT a substitute. This rule is mandatory because the bi-encoder and answer-LLM will not infer "replaced" from "has a new". Always emit the action profile; you MAY additionally emit a state profile, but the action profile is required.

   Concrete substitutions (apply these EXACTLY):
   - User says: `"I replaced the worn-out kitchen mat with a new one from IKEA"` → emit `user replaced their old kitchen mat with a new one from IKEA on <DATE>`. **Do NOT emit `user has a new kitchen mat from IKEA` as the only profile** — the "replaced" signal is lost.
   - User says: `"I have a new faucet"` (state-only, no verb) → emit `user has a new faucet`. State is fine here because no action verb was used.
   - User says: `"I got a new faucet to replace the old one"` (action-implied) → emit `user replaced their old kitchen faucet on <DATE>` AND optionally `user has a new faucet`.
   - User says: `"I donated my old coffee maker to Goodwill and got an upgrade"` → emit `user donated their old coffee maker to Goodwill on <DATE>` AND `user upgraded their coffee maker on <DATE>`. The donation IS a countable action.
   - User says: `"I finally fixed the kitchen shelves last weekend"` → emit `user fixed the kitchen shelves on <DATE>`.
   - User says: `"I had my AC serviced last month"` → emit `user had their AC serviced on <DATE>`.
   - User says: `"I replaced my old toaster with a toaster oven"` → emit `user replaced their old toaster with a toaster oven on <DATE>`. Optionally also `user has a toaster oven`.

   **Anti-pattern to avoid**: when the user describes an action on a possession, do NOT silently convert the verb to `has` or `got`. The mapping is:
   - `replaced X` → `user replaced X` (NOT `user has new X`)
   - `fixed X` → `user fixed X` (NOT `user's X is working again`)
   - `donated X` → `user donated X` (NOT just dropping it)
   - `got rid of X` → `user got rid of X` (NOT just dropping it)
...
8. **Incidental updates AND user-attribute framing facts.** When a session is mostly about topic A but the user mentions a fact about topic B in passing (a person, a place, a price, a count, a relationship, **a personal attribute**), still capture B as its own profile. Two distinct sub-rules apply:
   - **Incidental updates (mid-conversation asides):** Phrasings to watch for: "by the way", "actually", "now <X>", "<X> just <verb>ed", "moved back", "got re-approved", "ended up <verb>ing".
   - **User-attribute framing facts (volunteered as context for a different question):** When the user states a SPECIFIC NUMBER OR ATTRIBUTE about themselves, their family, their workplace, or their environment as conversational framing for a different topic, capture it as its own profile. Phrasings to watch for: "I'm <N> years old", "I'm <X> tall / weigh <X>", "my <family-member> is <N>", "the average <metric> in my <group> is <X>", "I make $<X>/year", "I have <N> kids/pets", "I've been here for <N> years", "considering my situation as a <demographic>". These are exactly the framing facts users later ask comparative questions about ("how much older am I than X", "what's my parents' average age", "compared to my department") — DO NOT skip just because the session's main topic is something else.
   - Examples:
     - "I'm currently 32 years old, can you recommend skincare products?" → emit `user is 32 years old` AND the skincare profiles. Do NOT only emit the skincare ones.
     - "Considering the average age of employees in my department is 29.5 years old, can you recommend master's programs?" → emit `user's department has an average employee age of 29.5 years old` AND the master's-program profiles.
     - "My mother is 55 and my father is 58 — they want to start a fitness routine." → emit `user's mother is 55 years old` AND `user's father is 58 years old` AND any fitness-routine profiles.
     - "I have two cats, what's a good vacuum?" → emit `user has two cats` AND any vacuum profiles.
