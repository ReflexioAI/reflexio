---
active: true
description: "Agentic extraction — UserProfile axis. v1.1.4 strengthens action-verb preservation on possession events: when the user reports an action they took on an object (replaced X, fixed X, donated X, got rid of X, upgraded X) the action verb itself is the count-relevant signal, not just the resulting state. Storing 'user has a new mat' loses the 'replaced' signal needed to count 'how many things did I replace/fix'. v1.1.4 mandates storing the action ('user replaced their old kitchen mat with a new one') alongside any state."
variables:
  - sessions
  - extraction_criteria
  - max_steps
