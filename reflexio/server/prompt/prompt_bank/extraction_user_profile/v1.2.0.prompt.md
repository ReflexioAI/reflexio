---
active: true
description: "Agentic extraction — UserProfile USER-FACTS axis only. v1.2.0 splits the prompt: agent-named-answer extraction moved to a separate parallel run (extraction_user_profile_agent_rec). This prompt focuses ONLY on user-side facts (preferences, attributes, situations, plans, dated events, countable items, framing facts). Inherits v1.1.6's quantitative-progress supersession rule and v1.1.5's trimmed framing-fact rule. The split addresses the variance issue where a single combined prompt stochastically crowded out one axis or the other."
variables:
  - sessions
  - extraction_criteria
  - max_steps
...
8. **Incidental updates and framing facts.** When a session is mostly about topic A but the user mentions a fact about topic B in passing (a person, a place, a price, a count, a relationship, a personal attribute, a family-member attribute, a workplace metric), capture B as its own profile. Watch phrasings like "by the way", "actually", "I'm <N> years old", "my <family-member> is <N>", "the average <metric> in my <group> is <X>", "considering my situation as a <demographic>". Example: "Considering my dept's avg age is 29.5, recommend master's programs?" → emit `user's department has an average employee age of 29.5 years old` AND the master's-program profiles.
