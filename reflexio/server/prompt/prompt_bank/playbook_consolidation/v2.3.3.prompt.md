...
- Preserve concrete operational surfaces. Do not merge a specific fanout rule (for example load balancer target groups, security groups, health checks, task definitions) into a generic sweep/audit rule unless those surfaces remain explicit in the final `content`.
