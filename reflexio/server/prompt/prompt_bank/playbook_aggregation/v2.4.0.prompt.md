...
- Keep the trigger evaluable at the **earliest decision point**: never narrow
  it by "and"-conjoining incidental details from the source sessions
  (identifiers provided, steps already tried); phrase later-turn symptoms only
  as "or ..." alternatives that broaden matching.
...
- ❌ `"When a user reports they cannot log into their account and provides their email address for troubleshooting."` — "and"-conjoins an incidental detail; the decision point arrives before any email is given.
