    ...
    def search_user_playbooks(  # noqa: C901
        ...
        if query:
            ...
            if _uses_unicode_lexical_fallback(query):
                unicode_sql = f"""SELECT up.* FROM user_playbooks_unicode_fts uf
                              JOIN user_playbooks up ON up.rowid = uf.rowid
                              WHERE user_playbooks_unicode_fts MATCH ?{where_extra}
                              ORDER BY bm25(user_playbooks_unicode_fts)
                              LIMIT ?"""
                ...
                unicode_rows = _rank_unicode_lexical_rows(
                    unicode_candidates,
                    query,
                    ("trigger", "content", "rationale", "source"),
                    overfetch,
                )
