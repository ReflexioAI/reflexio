...
| `search/` | Deep (agentic) unified search tier, opt-in via `UnifiedSearchRequest.search_depth="deep"`: `deep_search_service.py` orchestrates `planner.py` (LLM subquery planning incl. time windows + wants_current) → `executor.py` (parallel per-arm fan-out + fusion) → `reflector.py` (sufficiency grade + ranking, ≤1 corrective round) → `reranker.py` (listwise rerank after corrective rounds). Current-value questions get deterministic superseded/expired filtering + near-duplicate freshness collapse. Falls back to classic on any failure. |
