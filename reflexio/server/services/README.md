...
| `search_metering_worker.py` | Bounded process-local search-metering queue. Four daemon workers persist `search_request` and `learning_applied` after the response path, with fail-open drops, a five-second shutdown drain, and trace-linked `search.metering` transactions. |
